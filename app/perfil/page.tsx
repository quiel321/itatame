...
// FUNCAO PARA CONVERTER A CHAVE VAPID PARA O NAVEGADOR
  ...
  // FUNCAO QUE PEDE PERMISSAO E SALVA NO BANCO
  const inscreverParaNotificacoes = async () => {
    ...
    try {
      ...
      // 2. Registra e espera o Carteiro acordar
      const registration = await navigator.serviceWorker.register('/sw.js');
      await navigator.serviceWorker.ready;

      // 3. Limpa assinatura antiga se ja existir uma inscricao quebrada.
      const existingSub = await registration.pushManager.getSubscription();
      if (existingSub) {
        await existingSub.unsubscribe();
        console.log("Inscrição antiga destruída.");
      }

      const vapidKey = process.env.NEXT_PUBLIC_VAPID_PUBLIC_KEY;
      if (!vapidKey) throw new Error("A CHAVE VAPID sumiu ou não foi lida do .env.local!");

      // 4. Cria a nova assinatura limpa
          ...
          <div className="relative mb-4 mt-2 group cursor-pointer flex-shrink-0" onClick={() => document.getElementById('upload-foto')?.click()}>
            ...
            <div className={`w-24 h-24 bg-zinc-800 rounded-full flex items-center justify-center text-4xl font-black text-zinc-400 shadow-inner border-[3px] border-[#0a0a0e] ring-2 transition-all duration-300 overflow-hidden relative ${role === 'professor' ? 'ring-yellow-500/50 group-hover:ring-yellow-400' : 'ring-zinc-700/50 group-hover:ring-cyan-500/50'}`}>
              ...
              {uploadingFoto ? (
                ...
                <span className={`text-[9px] font-bold uppercase tracking-widest animate-pulse ${role === 'professor' ? 'text-yellow-400' : 'text-cyan-400'}`}>Enviando</span>
...
<button onClick={inscreverParaNotificacoes} className="cursor-pointer mt-3 w-full bg-blue-600/20 border border-blue-500/50 text-blue-400 hover:bg-blue-600 hover:text-white py-2.5 rounded-xl font-bold transition-all text-[11px] flex justify-center items-center gap-2">
    Ativar Alertas no Celular
        ...
        <div className={`bg-[#0a0a0e] border ${role === 'professor' && (abaAtiva === 'equipe' || abaAtiva === 'resumo' || abaAtiva === 'carteirinha') ? 'border-yellow-500/20' : 'border-white/5'} rounded-3xl p-4 md:p-6 shadow-xl min-h-[500px] w-full transition-colors duration-500`}>
          ...
          {/* 🎫 NOVA ABA: CARTEIRA DO ATLETA (IBJJF STYLE) */}
                 ...
                 <div className="flex gap-3 px-4 relative z-10">
                    ...
                    <div className="flex-1 space-y-1.5 flex flex-col justify-center">
                         ...
                         <div className="flex bg-white h-6 items-center overflow-hidden rounded-[2px] shadow-sm">
                            ...
                            <div className="text-black text-[9px] font-bold px-1 truncate flex-1">{nascimento ? formatarData(nascimento) : "--/--/----"}</div>
                         ...
                         <div className="flex bg-white h-6 items-center overflow-hidden rounded-[2px] shadow-sm">
                            ...
                            <div className="text-black text-[9px] font-bold px-1 truncate flex-1">{userId.substring(0,8).toUpperCase()}</div>
                         ...
                         <div className="flex bg-white h-6 items-center overflow-hidden rounded-[2px] shadow-sm">
                            ...
                            <div className="text-black text-[9px] font-bold px-1 truncate flex-1">31/12/{new Date().getFullYear()}</div>
          ...
          {abaAtiva === "resumo" && (
            ...
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 w-full">
              ...
              <div className="grid grid-cols-2 md:grid-cols-5 gap-2 md:gap-2.5 mb-6 w-full">

                {role === "professor" ? (

                  <>

                    <div className="bg-black/50 border border-white/5 rounded-2xl p-3 md:p-4 flex flex-col items-center justify-center transition-all duration-300 hover:bg-white/10 hover:-translate-y-1 cursor-default">

                      <span className="text-2xl font-black text-white">{totalAlunos}</span>

                      <span className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest mt-1">Alunos Ativos</span>

                    </div>

                    <div className="bg-[#fff9e6]/5 border border-yellow-500/30 rounded-2xl p-3 md:p-4 flex flex-col items-center justify-center relative overflow-hidden transition-all duration-300 hover:bg-yellow-500/10 hover:-translate-y-1 group cursor-default">

                      <span className="text-2xl font-black text-yellow-500">{ourosEquipe}</span>

                      <span className="text-[9px] font-bold text-yellow-600 uppercase tracking-widest mt-1">Ouros Equipe</span>

                    </div>

                    <div className="bg-[#f4f4f5]/5 border border-zinc-400/30 rounded-2xl p-3 md:p-4 flex flex-col items-center justify-center relative overflow-hidden transition-all duration-300 hover:bg-zinc-400/10 hover:-translate-y-1 group cursor-default">

                      <span className="text-2xl font-black text-zinc-300">{pratasEquipe}</span>

                      <span className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest mt-1">Pratas Equipe</span>

                    </div>

                    <div className="bg-[#fff1f2]/5 border border-orange-700/40 rounded-2xl p-3 md:p-4 flex flex-col items-center justify-center relative overflow-hidden transition-all duration-300 hover:bg-orange-700/10 hover:-translate-y-1 group cursor-default">

                      <span className="text-2xl font-black text-orange-500">{bronzesEquipe}</span>

                      <span className="text-[9px] font-bold text-orange-700 uppercase tracking-widest mt-1">Bronzes Equipe</span>

                    </div>

                    <div className="col-span-2 md:col-span-1 bg-yellow-900/10 border border-yellow-500/30 rounded-2xl p-3 md:p-4 flex flex-col items-center justify-center relative overflow-hidden transition-all duration-300 hover:-translate-y-1 hover:bg-yellow-500/10 group cursor-default">

                      <span className="text-2xl font-black text-yellow-500">{ourosEquipe + pratasEquipe + bronzesEquipe}</span>

                      <span className="text-[9px] font-bold text-yellow-600 uppercase tracking-widest mt-1">Total Medalhas</span>
                  ...
                  <>
                    ...
                    <div className="col-span-2 md:col-span-1 bg-cyan-900/10 border border-cyan-500/30 rounded-2xl p-3 md:p-4 flex flex-col items-center justify-center relative overflow-hidden transition-all duration-300 hover:-translate-y-1 hover:bg-cyan-500/10 group cursor-default">
