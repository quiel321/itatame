            ...
            {role === "professor" ? (
              ...
              <>
                ...
                <button onClick={() => setAbaAtiva("inscricoes")} className={`cursor-pointer w-full py-2.5 px-4 rounded-xl text-[11px] font-bold text-left transition-colors flex justify-between items-center ${abaAtiva === "inscricoes" ? "bg-white/10 text-white border border-white/5 shadow-sm" : "text-zinc-500 hover:text-white hover:bg-white/5"}`}>
                  ...
                  {minhasInscricoes.length > 0 && <span className="bg-red-600 text-white text-[9px] px-1.5 py-0.5 rounded-full pointer-events-none">{minhasInscricoes.length}</span>}
                </button>
              </>
            )}

            <button onClick={() => window.location.href = "/"} className="cursor-pointer text-zinc-500 hover:text-white hover:bg-white/5 py-2.5 px-4 rounded-xl text-[11px] font-medium text-left transition-colors mb-4 flex items-center gap-3 mt-2">
            ...
            <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 w-full">
              <h3 className="text-lg font-black text-white mb-4 flex items-center gap-2">
                {role === "professor" ? (
                  ...
                  <><svg className="w-5 h-5 text-cyan-500 pointer-events-none" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6"></path></svg> Resumo da Carreira</>
                )}
              </h3>
