...
export default function PerfilOrganizacaoCliente() {
    ...
    return (
      <FotosShell>
         <main className="min-h-screen bg-[#050505] flex items-center justify-center flex-col gap-4">
            <Loader2 size={40} className="text-retratt animate-spin" />
    ...
    return (
      <FotosShell>
         <main className="min-h-screen bg-[#050505] flex items-center justify-center flex-col gap-4">
            ...
            <Link href="/fotos" className="mt-4 bg-retratt text-black px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest cursor-pointer">Voltar aos Eventos</Link>
         </main>
      </FotosShell>
    );
  }
            ...
            <div className="flex flex-col md:flex-row items-center md:items-end gap-5 text-center md:text-left">
              ...
              <div className="w-36 h-36 md:w-44 md:h-44 rounded-full bg-[#111] border-[6px] border-[#050505] flex items-center justify-center overflow-hidden shadow-2xl shrink-0 relative group cursor-pointer">
                {organizador.avatar_url ? (
                  ...
                  <div className="w-full h-full bg-gradient-to-br from-retratt to-retratt flex items-center justify-center group-hover:scale-105 transition-transform duration-500">
              ...
              <div className="mb-2 md:mb-4">
                <div className="flex items-center justify-center md:justify-start gap-2 mb-1">
                   <Building2 size={16} className="text-retratt" />
                   <p className="text-[10px] font-black uppercase tracking-[0.2em] text-retratt">Organizador Verificado</p>
                </div>
                ...
                <p className="text-xs md:text-sm text-zinc-300 mt-2 flex items-center justify-center md:justify-start gap-1.5 font-medium drop-shadow-md">
                  <MapPin size={14} className="text-retratt" /> {organizador.localizacao || "Brasil"}
                </p>
              </div>
            </div>
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-6 mb-10">
            ...
            <div className="w-full md:w-[380px] relative group">
              <div className="absolute inset-y-0 left-4 flex items-center pointer-events-none">
                <Search size={16} className="text-zinc-500 group-focus-within:text-retratt transition-colors" />
              </div>
              <input
                type="text"
                value={busca}
                ...
                className="w-full bg-[#0a0a0e] border border-white/10 rounded-2xl py-3.5 pl-11 pr-4 text-xs font-medium text-white outline-none focus:border-retratt/50 focus:ring-1 focus:ring-retratt/50 transition-all placeholder:text-zinc-600"
              />
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
              {eventosFiltrados.map((evt) => (
                <Link
                  ...
                  key={evt.id}
                  className="group flex flex-col bg-[#0a0a0e] border border-white/5 rounded-3xl overflow-hidden hover:border-retratt/30 transition-all duration-500 cursor-pointer shadow-lg hover:shadow-[0_10px_40px_rgba(255,90,31,0.08)]"
                >
                  ...
                  <div className="p-5 flex flex-col flex-1 relative">
                    <h3 className="text-sm font-black uppercase tracking-tight text-white mb-4 line-clamp-2 leading-snug group-hover:text-retratt transition-colors">
                      {evt.nome}
                    </h3>
                    ...
                    <div className="mt-auto pt-4 border-t border-white/5 flex items-center justify-between">
                      <span className="text-[10px] font-black uppercase tracking-widest text-retratt group-hover:text-retratt transition-colors">Ver Galeria Oficial</span>
                      <div className="w-8 h-8 rounded-full bg-white/5 group-hover:bg-retratt text-zinc-500 group-hover:text-black flex items-center justify-center transition-colors">
