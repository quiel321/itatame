      ...
      <main className="min-h-screen bg-[#050505] text-white">
          ...
          <div className="mx-auto max-w-7xl px-4 py-8 md:px-8 md:py-12">
            <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
              <div>
                ...
                <p className="mt-3 max-w-2xl text-xs leading-5 text-zinc-400 md:text-sm">O líquido usa o valor efetivamente recebido informado pelo Mercado Pago, depois da comissão Retratt, do royalty do organizador e da tarifa do pagamento.</p>
              </div>
        ...
        <section className="mx-auto max-w-7xl space-y-8 px-4 py-8 md:px-8">
          {carregando && !dados ? (
            ...
            <div className="rounded-3xl border border-retratt/30 bg-retratt/10 p-6 text-sm text-red-200">{erro}</div>
          ) : dados && (
            <>
              {dados.vendasSemConfirmacao > 0 && (
                <div className="rounded-2xl border border-retratt/30 bg-retratt/10 p-4 text-xs leading-5 text-orange-100">{dados.vendasSemConfirmacao} venda(s) aguardam confirmação de tarifa no Mercado Pago. Elas aparecem no histórico, mas não entram no total líquido até a confirmação.</div>
              )}

              <div className="grid grid-cols-2 gap-3 lg:grid-cols-3 xl:grid-cols-6">
                {[
                  [ReceiptText, "Vendas pagas", String(dados.resumo.vendas), "text-white"],
                  [ImageIcon, "Fotos vendidas", String(dados.resumo.fotos), "text-retratt"],
                  [WalletCards, "Faturamento bruto", moeda(dados.resumo.brutoCentavos), "text-white"],
                  [Camera, "Comissão Retratt", `- ${moeda(dados.resumo.itatameCentavos)}`, "text-retratt"],
                  [Store, "Royalties", `- ${moeda(dados.resumo.royaltyCentavos)}`, "text-retratt"],
              ...
              <div className="rounded-3xl border border-emerald-500/20 bg-emerald-500/[0.04] p-5 md:p-6">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                  <div><p className="text-[9px] font-black uppercase tracking-widest text-emerald-300">Composição do líquido</p><p className="mt-2 text-xs text-zinc-400">Bruto {moeda(dados.resumo.brutoCentavos)} · Retratt -{moeda(dados.resumo.itatameCentavos)} · Royalties -{moeda(dados.resumo.royaltyCentavos)} · Mercado Pago -{moeda(dados.resumo.taxaMercadoPagoCentavos)}</p></div>
              ...
              <div>
                <div className="mb-4 flex items-end justify-between gap-3"><div><p className="text-[9px] font-black uppercase tracking-widest text-retratt">Controle por galeria</p><h2 className="mt-1 text-xl font-black uppercase">Resultado de cada trabalho</h2></div></div>
              ...
              <div>
                ...
                <div className="mt-4 space-y-3">
                  {dados.vendas.map((venda) => <article key={venda.id} className="rounded-2xl border border-white/5 bg-[#0a0a0e] p-4 md:p-5"><div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><p className="truncate text-sm font-black uppercase">{venda.galeria}</p>{venda.confirmadoPeloMercadoPago && <span className="inline-flex items-center gap-1 rounded-full bg-emerald-500/10 px-2 py-1 text-[7px] font-black uppercase tracking-wider text-emerald-300"><CheckCircle2 size={10} /> Confirmado</span>}</div><p className="mt-1 text-[9px] uppercase tracking-wider text-zinc-500">{dataHora(venda.data)} · {venda.fotos} {venda.fotos === 1 ? "foto" : "fotos"} · {venda.metodo || "Tarifa em consulta"}</p></div><div className="grid grid-cols-2 gap-2 sm:grid-cols-5 lg:min-w-[600px]"><div className="rounded-xl bg-black/50 p-2.5"><p className="text-[7px] font-black uppercase text-zinc-600">Bruto</p><p className="mt-1 text-sm font-black">{moeda(venda.brutoCentavos)}</p></div><div className="rounded-xl bg-black/50 p-2.5"><p className="text-[7px] font-black uppercase text-zinc-600">Retratt</p><p className="mt-1 text-sm font-black text-retratt">-{moeda(venda.itatameCentavos)}</p></div><div className="rounded-xl bg-black/50 p-2.5"><p className="text-[7px] font-black uppercase text-zinc-600">Royalty</p><p className="mt-1 text-sm font-black text-retratt">-{moeda(venda.royaltyCentavos)}</p></div><div className="rounded-xl bg-black/50 p-2.5"><p className="text-[7px] font-black uppercase text-zinc-600">Mercado Pago</p><p className="mt-1 text-sm font-black text-zinc-300">{venda.taxaMercadoPagoCentavos === null ? "Consultando" : `-${moeda(venda.taxaMercadoPagoCentavos)}`}</p></div><div className="col-span-2 rounded-xl bg-emerald-500/10 p-2.5 sm:col-span-1"><p className="text-[7px] font-black uppercase text-emerald-500/60">Líquido</p><p className="mt-1 text-sm font-black text-emerald-400">{venda.liquidoCentavos === null ? "Pendente" : moeda(venda.liquidoCentavos)}</p></div></div></div></article>)}
